...
public partial class CustomView : UserControl
{
    ...
    {
        InitializeComponent();
        // Do NOT write code here, instead use OnLoaded method
    }

    private void OnLoaded(object? sender, Avalonia.Interactivity.RoutedEventArgs eventArgs)
    {
        ...
        richTextBox.TextChanged += (sender, eventArgs) =>
        {
            ...
            {
                errorMessage.Content = $"Cannot parse json: {e.Message}";
                return;
            }
        ...
        nullIfNotExistentCheckBox.IsCheckedChanged += (sender, eventArgs) =>
        {
            RerenderJson();
        };
        ignoreNullCheckBox.IsCheckedChanged += (sender, eventArgs) =>
        {
            RerenderJson();
        };
    }
